...
import { useBlockStoreActions } from "@/hooks/useBlockStore.js";
import {
  AddBlock,
  AddChildBlock,
  BlockTransformation,
...
export const useStoryContent = <T extends Block>(
  ...
  const getChildBlockTransformations = (
    block: Block,
    parent?: BlockID,
  ): BlockTransformation[] => {
    ...
    for (const childID of block.children) {
      ...
      if (child !== undefined) {
        transformations = transformations.concat(
          getChildBlockTransformations(child, block.uuid),
        );
      }
    }
  ...
  const createAndPersistContent = useCallback(() => {
    ...
    session.getBlocks().forEach((block: Block) => {
