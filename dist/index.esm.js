import React, { createContext, forwardRef, useContext, useEffect, useCallback, cloneElement, useRef, useState, isValidElement, createRef, PureComponent } from 'react';
...
const BlockFactoryContext = createContext({
    factory: undefined,
    setFactory: (factory) => { }
});
...
const BlockListComponent = forwardRef(({ className, list, selected, onSelected, key }, ref) => {
...
const ContentBlockComponent = forwardRef(({ className, content, selected, onSelected, onClick, key }, ref) => {
    const { factory, setFactory } = useContext(BlockFactoryContext);
    useEffect(() => {
        if (onSelected !== undefined) {
            onSelected(selected);
...
const ContentSectionComponent = forwardRef(({ className, section, selected, onSelected, onClick, key }, ref) => {
    const { factory, setFactory } = useContext(BlockFactoryContext);
    useEffect(() => {
        if (onSelected !== undefined) {
            onSelected(selected);
...
const ContentSpanComponent = forwardRef(({ className, span, selected, onSelected, onClick, key }, ref) => {
    useEffect(() => {
        if (onSelected !== undefined) {
            onSelected(selected);
        }
...
const NamedBlockComponent = forwardRef(({ className, content, collapsed, selected, onToggle, onSelected, onTransitionEnd, key }, ref) => {
    const { factory, setFactory } = useContext(BlockFactoryContext);
    useEffect(() => {
        if (onSelected !== undefined) {
            onSelected(selected);
...
const SentinalView = forwardRef(({ sentinal, selected, onSelected, key }, ref) => {
    useEffect(() => {
        if (onSelected !== undefined) {
            onSelected(selected);
        }
...
class DefaultBlockFactory {
    ...
    useCollapsed(block) {
        ...
        const toggleCollapsed = useCallback((c) => setCollapsed(!c), [collapsed]);
...
export { Base, BlockFactoryContext, BlockList, BlockListComponent, BlockListItem, BlockStream, Code, CollapsibleBlock, Content, ContentBlock, ContentBlockComponent, ContentSection, ContentSpan, DefaultBlockFactory, List, NamedBlock, NamedContent, Section, Selectable, SentinalView, Span, backgroundColor$3 as backgroundColor, borderColor$2 as borderColor, defaultFont, fontWeight, selectedBackgroundColor, selectedBorderColor, selectedTextColor, selectedVariants, textColor$3 as textColor };
